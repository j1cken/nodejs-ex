...
sh 'curl -i -s http://master.nodejs-integration.svc.cluster.local:8080 | head -1 |grep 200; exit $?'
...
stage 'test uat'
sh 'curl -i -s http://helloworld.nodejs-uat.svc.cluster.local:8080 | head -1 |grep 200; exit $?'
